...
public class ControllerImpl implements Controller{
    ...
    public BattleZone getBattleZoneByName(String battleZoneName) {
        return battleZones.stream().filter(battleZone -> battleZone.getName().equals(battleZoneName))
    ...
    public String addBattleshipToBattleZone(String battleZoneName, String shipType, String shipName, int health) {
        ...
        return SUCCESSFULLY_ADDED_SHIP.formatted(shipType,shipName,battleZone.getName());
    }
    ...
    public String startBattle(String battleZoneName, String attackingShip, String shipUnderAttack) {
        ...
        List<String> shipNames = battleZone.getShips().stream().map(Battleship::getName).toList();
    ...
    public String getStatistics() {
        ...
        for (BattleZone zone : battleZones){
            sb.append(SHIPS_IN_BATTLE_ZONE.formatted(zone.getName())).append("\n");
            if (zone.getShips().size() == 1){
                Battleship battleship = zone.getShips().iterator().next();
                sb.append(SHIP_WINS.formatted(battleship.getName()));
            } else {
                for (Battleship ship : zone.getShips()) {
                    sb.append(SHIP_INFO.formatted(ship.getName(),ship.getHealth(),ship.getAmmunition())).append("\n");
                }
            }
        }
